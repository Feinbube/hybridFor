    ...
    public class Crypt : ExampleBase
    {
        ...
        **************************************************************************/
        ...
        protected override void setup()
        {
            ...
            dict = new char[][] { "Osten3".ToCharArray(), "Bahnhof".ToCharArray() };
        ...
        protected override void algorithm()
        {
            ...
            Parallel.For(0, dict.Length, delegate(int dictId)
            {
                ...
                int length = memcpy_pg(password, dict[dictId], dict[dictId].Length);
                ...
                for (int i = 0; i < pwdEntryCount; i++)
                {
                    ...
                    bool result = pwCracked[i]; // __global byte* result = crkPwd + (i*9);
        ...
        private void memcpy_gp(byte[] dst, byte[] src, int len)
        {
            for (int i = 0; i < len; i++)
                dst[i] = src[i];
        }
        ...
        private void setkey(char[] key, char[] C, char[] D, char[] E, char[][] KS)
        {
            ...
            * each 8-bit byte is not used, so C and D are only 28 bits apiece.
            **********************************************************************/
            for (i = 0; i < 28; i++)
            {
                C[i] = key[PC1_C[i] - 1];
