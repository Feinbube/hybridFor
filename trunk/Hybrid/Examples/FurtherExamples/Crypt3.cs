...
{
    public class Crypt3 : ExampleBase
    {
        #region crypt 3 implementation
        ...
        **************************************************************************/
        ...
        static byte[] shifts = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
        ...
        * Returns:     none
        **************************************************************************/
        static void setkey(ref char[] key, ref char[] C, ref char[] D, ref char[] E, char[][] KS/*[16][48]*/)
        {
            int i, j, k, temp;
        ...
        * Returns:     none
        **************************************************************************/
        static void encrypt(ref char[] block, char[] E, char[][] KS/*[16][48]*/, char[] preS)
        {
            int i, ii, temp, j, k;
            ...
            for (ii = 0; ii < 16; ii++)
            {
                ...
                ******************************************************************/
                for (j = 0; j < 48; j++)
                    preS[j] = (char)(right[E[j] - 1] ^ KS[i][j]);
                ...
                ******************************************************************/
                for (j = 0; j < 8; j++)
                {
                    ...
                    f[temp + 0] = (char)((k >> 3) & 01);
                    f[temp + 1] = (char)((k >> 2) & 01);
                    f[temp + 2] = (char)((k >> 1) & 01);
                    f[temp + 3] = (char)((k >> 0) & 01);
                }
        ...
        **************************************************************************/
        public static void crypt(char[] pw, char[] salt, ref char[] iobuf)
        {
            int i, j, temp;
            char c;
            ...
            char[] block = new char[66];             /* 1st store key, then results */
            ...
            int pwIndex = 0;
            for (i = 0, c = pw[0]; (c != 0) && (i < 64); i++)
            {
                for (j = 0; j < 7; j++, i++)
                    block[i] = (char)((c >> (6 - j)) & 01);

                pwIndex++;
                if (pwIndex >= pw.Length)
                    break;
                c = pw[pwIndex];
            }

            /* set key based on pw */
            setkey(ref block, ref C, ref D, ref E, KS);
        ...
        #endregion crypt 3 implementation

        string crypt(string password, string salt)
        {
            char[] result = new char[12];
            crypt(password.ToCharArray(), salt.ToCharArray(), ref result);
        ...
        class PasswordEntry
        {
            ...
            {
                if(!CrackAttempted())
                    return Username + ":" + Salt + PasswordHash + "[" + OriginalPassword + "/" + "Not Cracked" + "]";
                else
                    if (IsCracked())
                        return Username + ":" + Salt + PasswordHash + "[" + OriginalPassword + "/" + "CRACKED" + "]";
                    else
                        return Username + ":" + Salt + PasswordHash + "[" + OriginalPassword + "/" + "WRONG(" + CrackedPassword + ")]";
            }
            ...
            {
                return CrackedPassword == OriginalPassword;
            }
        }
        ...
        private PasswordEntry randomPasswordEntry(int i)
        {
        ...
        protected override void printInput()
        {
            foreach (PasswordEntry passwordEntry in passwordEntries)
                Console.WriteLine(passwordEntry.ToString());
        }
        ...
        protected override void algorithm()
        {
            Parallel.For(ExecuteOn, 0, dictionary.Count, delegate(int j)
            {
                foreach (PasswordEntry passwordEntry in passwordEntries)
                {
                    if (passwordEntry.IsCracked())
                        continue;

                    string dictionaryEntry = dictionary[j];

                    string possiblePasswordHash = crypt(dictionaryEntry, passwordEntry.Salt);
                    ...
                    for(int i=0; i<10; i++)
                    {
                        string newDictionaryEntry = dictionaryEntry + i.ToString();
